...
public class EventManager {
	...
	public void onTextureStitch(TextureStitchEvent event){
		ResourceLocation particleGlow = new ResourceLocation("embers:entity/particleGlow");
	...
	public void onChunkGeneration(ChunkEvent.Load event){
		...
		if (!data.emberData.containsKey(""+event.getChunk().xPosition+" "+event.getChunk().zPosition)){
			...
			baseAmount *= mult;
			bonusAmount *= mult;
			data.emberData.put(""+event.getChunk().xPosition+" "+event.getChunk().zPosition, 4.0*(baseAmount+random.nextDouble()*bonusAmount));
			data.markDirty();
		}
	}
