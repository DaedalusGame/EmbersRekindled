...
public class TileEntityFluidPipeRenderer extends TileEntitySpecialRenderer<TileEntityFluidPipe> {
	...
	public void render(TileEntityFluidPipe tile, double x, double y, double z, float partialTicks, int destroyStage, float tileAlpha){
		if (tile != null){
            ...
            if (shouldRenderPipe(tile.south)){
	...
	private boolean shouldRenderLip(EnumPipeConnection connection) {
		return connection == EnumPipeConnection.BLOCK;
	}

	private boolean shouldRenderPipe(EnumPipeConnection connection) {
		return connection == EnumPipeConnection.PIPE || shouldRenderLip(connection);
	}
}
