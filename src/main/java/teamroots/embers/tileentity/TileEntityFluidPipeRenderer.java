...
public class TileEntityFluidPipeRenderer extends TileEntitySpecialRenderer<TileEntityFluidPipe> {
	...
	public TileEntityFluidPipeRenderer(){
		super();
	}
	
	@Override
	public void render(TileEntityFluidPipe tile, double x, double y, double z, float partialTicks, int destroyStage, float tileAlpha){
		if (tile != null){
            ...
            if (tile.up == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.up == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            	RenderUtil.addBox(buffer, up.x1+x, up.y1+y, up.z1+z, up.x2+x, up.y2+y, up.z2+z, up.textures, new int[]{1,1,1,1,1,1});
            }
            if (tile.down == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.down == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            	RenderUtil.addBox(buffer, down.x1+x, down.y1+y, down.z1+z, down.x2+x, down.y2+y, down.z2+z, down.textures, new int[]{-1,-1,1,1,1,1});
            }
            if (tile.north == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.north == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.south == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.south == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.west == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.west == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            	RenderUtil.addBox(buffer, west.x1+x, west.y1+y, west.z1+z, west.x2+x, west.y2+y, west.z2+z, west.textures, new int[]{1,1,1,1,1,1});
            }
            if (tile.east == TileEntityFluidPipe.EnumPipeConnection.PIPE || tile.east == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            	RenderUtil.addBox(buffer, east.x1+x, east.y1+y, east.z1+z, east.x2+x, east.y2+y, east.z2+z, east.textures, new int[]{1,1,1,1,-1,-1});
            }
            if (tile.up == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.down == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.north == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.south == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.west == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
            ...
            if (tile.east == TileEntityFluidPipe.EnumPipeConnection.BLOCK){
