...
public class TileEntityItemPumpRenderer extends TileEntitySpecialRenderer {
	public ResourceLocation texture = new ResourceLocation(Embers.MODID + ":textures/blocks/itemPipeTex.png");
	...
	public TileEntityItemPumpRenderer(){
		super();
	}
	...
	public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTicks, int destroyStage){
		if (tile instanceof TileEntityItemPump){
			TileEntityItemPump pipe = (TileEntityItemPump)tile;
            ...
            if (pipe.up == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.up == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.up == TileEntityItemPump.EnumPipeConnection.LEVER){
            	RenderUtil.addBox(buffer, up.x1+x, up.y1+y, up.z1+z, up.x2+x, up.y2+y, up.z2+z, up.textures, new int[]{1,1,1,1,1,1});
            }
            if (pipe.down == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.down == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.down == TileEntityItemPump.EnumPipeConnection.LEVER){
            	RenderUtil.addBox(buffer, down.x1+x, down.y1+y, down.z1+z, down.x2+x, down.y2+y, down.z2+z, down.textures, new int[]{-1,-1,1,1,1,1});
            }
            if (pipe.north == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.north == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.north == TileEntityItemPump.EnumPipeConnection.LEVER){
            ...
            if (pipe.south == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.south == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.south == TileEntityItemPump.EnumPipeConnection.LEVER){
            ...
            if (pipe.west == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.west == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.west == TileEntityItemPump.EnumPipeConnection.LEVER){
            	RenderUtil.addBox(buffer, west.x1+x, west.y1+y, west.z1+z, west.x2+x, west.y2+y, west.z2+z, west.textures, new int[]{1,1,1,1,1,1});
            }
            if (pipe.east == TileEntityItemPump.EnumPipeConnection.PIPE || pipe.east == TileEntityItemPump.EnumPipeConnection.BLOCK || pipe.east == TileEntityItemPump.EnumPipeConnection.LEVER){
